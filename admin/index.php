...
<?php 
	if(!isset($_SESSION['admin_logged_in'])){
		...
		?>
		<?php 
		$sql = "SELECT * FROM applications a INNER JOIN customers c ON a.cus_id = c.id INNER JOIN internships i ON a.int_id = i.id WHERE a.applied = 1";
...
<body onload="animation()">
	...
	<main>
		...
		<a href="./updateStatus.php?all=1" class="float-right btn btn-sm btn-primary" name="approve">
		<i class="fas fa-check-double"> Approve all</i>
				</a> 
	...
	<tr>
		<?php 
			//if($application['interview_date'] > $currentdate){
				...
				$sql1 = "select int_id workstatus from applications a inner join customers c on a.cus_id = c.id where workstatus = 'interning' and email = '$email'";
		...
		<td class="text-center">
			<a href="./updateStatus.php?id=<?php echo $application['id']?>" class=" m-1 text-white btn btn-sm btn-primary" name="approve">
			<?=$application['status'] . "...";?>
		</a>
...
<td class="text-center">
	...
	<select class="form-select m-2" aria-label="Default select example" name="workstatus" id="value">
